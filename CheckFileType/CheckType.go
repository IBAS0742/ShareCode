...
func init() {
	...
	fileTypeMap.Store("7B5C727466315c616e73", "rtf")  // 我（IBAS）看到的rtf
	...
	fileTypeMap.Store("504b03040a0000000000", "zip") // 我（IBAS）看到的zip
...
func CheckType(filePath string) string {
	return CheckTypeWithLog(filePath,log.New(os.Stderr, "", log.LstdFlags))
}
func CheckTypeWithLog(filePath string,logger * log.Logger) string {
	...
	oType := getFileType(fSrc[:48])
	if oType == "docx" {
		return testOfficeXType(filePath)
	} else if oType == "wps" {
		return testOffice03Type(fSrc)
	} else {
		return oType
	}
}

func testOfficeXType(filename string) string {
...
func testOffice03Type(buf []byte) string {
	xls := strings.ToLower(`FDFFFFFF02`)
	word := strings.ToLower(`ECA5C100`)
	ppt := strings.ToLower(`FDFFFFFFA5`)
	s := bytesToHexString(buf[512:522])
	/*if xls == s[:len(xls)] {
		return "xls"
	} else if ppt == s[:len(ppt)] {
		return "ppt"
...
func TestGetRange(filePath string,start,end int) string {
	f, err := os.Open(filePath)
	if err != nil {
		panic(err)
	}
	...
	return bytesToHexString(fSrc[start : end])
}